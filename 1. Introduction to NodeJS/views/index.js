...
<body>
    ...
    <main>
        <section class="cats">
            <ul>
               ${cats.map(x => `<li>
               <img src="${x.imageUrl}" alt="${x.breed}">
               <h3>${x.name}</h3>
               <p><span>Breed: </span>${x.breed}</p>
               <p><span>Description: </span>${x.description}</p>
               <ul class="buttons">
                   <li class="btn edit"><a href="http://localhost:5000/cats/${x.id}/edit/">Change Info</a></li>
                   <li class="btn delete"><a href="http://localhost:5000/cats/${x.id}/delete">New Home</a></li>
               </ul>
           </li>`).join('')}
            </ul>
        </section>
